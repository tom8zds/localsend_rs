...
/// Run the panel server until the process exits. `db_path` overrides
/// PANEL_DB. Returns only on a fatal startup error.
pub async fn serve(cfg: state::PanelConfig, db_path: String) -> std::io::Result<()> {
    let state = std::sync::Arc::new(AppState::with_db(cfg, db_path)?);
    ...
    let bind: SocketAddr = std::env::var("PANEL_BIND")
        ...
        .parse()
        .map_err(|e| std::io::Error::other(format!("PANEL_BIND must be ip:port: {e}")))?;
